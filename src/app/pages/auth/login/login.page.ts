...
export class LoginPage implements OnInit {
  ...
  login() {
    ...
    this.firebaseSvc.Login(user).then(
      (res) => {
    ...
    let ref = this.firebaseSvc
      ...
      .subscribe(
        (res: any) => {
          ...
          let deviceExist = res.devices.filter(
            (device) => device.uuid == currentDevice.uuid
          ).length;
          ...
          if (deviceExist) {
            ...
            if (res.devices.length == 3) {
              this.utilsSvc.presentToast(
                "No se pudo iniciar. Tienes 3 dispositivos vinculados a esta cuenta, elimina alguno para iniciar en este."
              );
            } else {
  ...
  getLicense(user: User) {
    ...
    let ref = this.firebaseSvc
      .getCollectionConditional("licenses", (ref) =>
  ...
  updateDevices(user: User) {
    ...
    this.firebaseSvc.UpdateCollection("users", data).then(
      (res) => {
        this.loading = false;
        this.utilsSvc.saveLocalStorage("user", user);
